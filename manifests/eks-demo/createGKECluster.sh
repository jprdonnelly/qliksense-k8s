...
# Display Regions - Store as var
gcloud compute regions list
echo
echo "Please choose an available region"
read REGION
# Create Cluster
gcloud compute zones list --filter="name:('REGION')"
echo
echo "Please choose an available zone from your chosen region"
read ZONE
echo
echo Region: $REGION
echo Zone: $ZONE
gcloud container clusters create $QLIKSENSE_HOST --zone $ZONE --no-enable-basic-auth --cluster-version "1.15.9-gke.24" --machine-type "n1-standard-16" --image-type "UBUNTU" --disk-type "pd-standard" --disk-size "100" --metadata disable-legacy-endpoints=true --num-nodes "4" --enable-stackdriver-kubernetes --enable-ip-alias --default-max-pods-per-node "110" --no-enable-master-authorized-networks --addons HorizontalPodAutoscaling,HttpLoadBalancing --enable-autoupgrade --enable-autorepair
echo "Cluster"
echo "--"
